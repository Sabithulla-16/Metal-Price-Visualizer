...
async function usd(sym){
  const res = await fetch(`/.netlify/functions/metals?symbol=${sym}`);
  const data = await res.json();
  return data.price;
}
...
function storeDaily(usdPrice, basePrice, assetType, currentMode){
  // Store today's price for all asset types (metals and crypto) across all modes
  const key="dailyPrice_"+symbol;
  const today=new Date().toISOString().split("T")[0];
  const stored=JSON.parse(localStorage.getItem(key)||"{}");
  if(stored.date!==today){
    localStorage.setItem(key,JSON.stringify({date:today, usdPrice, basePrice, type: assetType, mode: currentMode}));
  }
}
...
async function getSavedDailyPrices(sym) {
  try {
    ...
    // Append today's price from localStorage if it exists
    const todayKey = "dailyPrice_" + sym;
    const todayData = JSON.parse(localStorage.getItem(todayKey) || "{}");
    const today = new Date().toISOString().split("T")[0];
    if (todayData.date === today && todayData.basePrice !== undefined) {
      const alreadyExists = result.find(x => x.date === today);
      if (!alreadyExists) {
        result.push({ date: today, price: todayData.basePrice });
...
async function renderCharts(){
  try {
    ...
    // Render comparison chart - all metals today
    const monthlyChartEl = document.getElementById('monthlyChart');
    if (monthlyChartEl) {
      const res = await fetch("./data/daily_prices.json");
      const fullData = await res.json();
      const today = new Date().toISOString().split("T")[0];
      const todayData = fullData[today] || {};
      const metals = ['xau', 'xag', 'xpt', 'xpd', 'hg'];
      const metalNames = ['Gold', 'Silver', 'Platinum', 'Palladium', 'Copper'];
      const comparisonValues = metals.map(m => todayData[m] || 0);
      const colors = ['#FFD700', '#C0C0C0', '#E5E4E2', '#71797E', '#B87333'];
